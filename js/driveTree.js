	...
	$.fn.extend({
		...
		checktree : function() {
			...
			var checkParents = function(c) {
				...
				if (parentLi.length) {
					var siblingsChecked = parseInt($('input[type="checkbox"]:checked', c.parents('ul:eq(0)')).length), rootCheckbox = parentLi.find('input[type="checkbox"]:eq(0)');
...
$(document).ready(function() {
	...
	$(document).on("click", ".driveTreeCollapse", function(event) {
		event.preventDefault();

		$(this).next().next().hide("slow")
		$(this).attr("class", "driveTreeExpand");
	})
	$(document).on("click", ".driveTreeExpand", function(event) {
		event.preventDefault();

		$(this).next().next().show("slow");
		$(this).attr("class", "driveTreeCollapse");
	})
